...
internal class DeleteOrderTests : SingleRoleBaseTestSetup
{
    ...
    public async Task Delete_Unavailable_Order_ByPublisher(string status)
    {
        ...
        Order statusChangedOrderResult = await _laboratory.ChangeOrderStatusAsync(orderResult, new Skyware.Arenal.ApiModel.Model.Actions.OrderStatusRequest() { NewStatus = status });
